...
8. 用户点击 `Summary ready` 后，底部弹层显示 AI 摘要，并可复制、重新生成和删除 summary；重新生成过程中退出 sheet 不会取消任务，timeline 显示轻量状态，成功后替换旧摘要，失败时旧摘要仍保留且可查看失败原因。原 post/media/legacy transcript metadata/comments 不被修改。
