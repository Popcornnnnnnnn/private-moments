...
- 支持通过月份菜单快速跳转。
- 日期跳转保持月级粒度，只显示已有内容的月份；不在 Calendar 菜单里展开每日列表，不做完整 Calendar tab 或 Archive 管理中心。
- 精确日期查找后续交给搜索增强承担，例如模糊搜索、日期搜索，甚至 AI 智能搜索，而不是继续加重日历界面。
- 月份文案偏生活感，例如 `April 2026` 或菜单里的 `Apr 2026`；不显示每天数量统计，不使用数据库感的纯日期格式作为主文案。
