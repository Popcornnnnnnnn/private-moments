...
- 可以手动修改日期时间。
- 发布成功后内容立即出现在本地时间线。
- 输入体验可以提供轻量辅助，例如 `- `、`• `、`1. ` 的列表延续；保存格式保持 plain text `String`。Timeline/Detail 只允许展示层识别行首 `# ` 和 `## ` 标题；Composer/Edit 保持原文输入，不支持 bold、quote 或 link preview。
...
- Timeline 可组合筛选内容类型、月份、收藏、评论、待同步；有搜索词时可再按命中来源筛选，并显示 active filter chips 和 `Clear`。
- 搜索和筛选状态不跨 App 重启持久化。
- 离线时可搜索本地已有文本。
...
- Composer 只提供可选主标签，不要求发布时必须选择标签；主题标签放在详情/编辑和 Settings 中维护。
- 主时间线最多只显示一个主标签，并可通过 Settings > Feature Modules > `Show Tags in Timeline` 隐藏；隐藏后标签数据、搜索、筛选和编辑继续工作。
...
- Filter 菜单可以按主标签和主题标签筛选，并和内容类型、月份、收藏、评论、待同步等条件组合。
- Settings > Tags 可以查看标签 usage count、新增自定义标签、重命名/归档/恢复 topic、批量归档/合并 Topic、批量恢复/永久删除 Archived、管理 alias，并自定义主标签颜色；默认主标签不可重命名、归档或删除。
...
- 摘要语言跟随原语音/视频的主导语言，专有名词和英文术语保持原样。
...
- 语音在发布页内录制为 AAC/M4A，最长 60 分钟，支持暂停、继续、停止和发布前试听。
