...
Private Moments 是一个单用户、私密、自托管的生活记录产品。它的核心体验类似私人版朋友圈：用户可以在 iPhone 上随手发布文字、图片、语音或短视频，内容先稳定保存在本机，即使离线也不丢；当 iPhone 通过 Tailscale 或私有 VPN 连回 Mac 后，内容自动同步到 Mac 服务端作为长期归档源。
...
- 支持置顶少量重要 moments，在时间线顶部以默认折叠的标题列表快速回到原动态，同时不破坏原本的时间顺序。
...
- Day Review 内部可用轻量横向 chips 多选过滤当天内容：Photos、Audio、Video、Favorites、Comments 是 OR 关系，点已选 chip 可取消，点 All 清空筛选。Summary 不作为独立筛选项；时间列使用 24 小时制，并用很轻的 Morning/Afternoon/Evening/Late Night 分隔让一天更有节奏。
...
- 时间线顶部在存在可见置顶内容时显示 `Pinned` 区域。
- `Pinned` 默认折叠，只显示每条置顶 moment 的单行标题。
- 置顶 moment 的标题优先使用正文里的 `#` / `##` 标题；没有标题时使用第一条正文、AI summary title 或媒体/日期 fallback。
- 点击置顶标题打开原 moment detail。
- 展开 `Pinned` 后可以查看更完整的预览，但默认状态仍是标题列表。
- 原 moment 仍保留在正常时间顺序中；Calendar、Day Review、搜索、筛选和 Weekly Review 不因为置顶而改变发生时间或统计。
- Pin / Unpin 入口放在详情页、context menu 或其他低频操作里，不在每条时间线 row 常驻增加按钮。
- Pin 状态应该随同步恢复，离线 pin/unpin 后在 Mac 可达时同步。
- 第一个版本不支持自定义置顶标题、手动拖拽排序、置顶评论、置顶 review 或 Admin 侧管理。
...
- AI 不为每条判断绑定 evidence；只在 `Worth Revisiting` 中提供低权重 moment anchors，帮助用户在当前 Review 界面内回到原始 moment。
