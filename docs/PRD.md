...
Private Moments 是一个单用户、私密、自托管的生活记录产品。它的核心体验类似私人版朋友圈：用户可以在 iPhone 上随手发布文字、图片、语音或短视频，内容先稳定保存在本机，即使离线也不丢；当 iPhone 通过作者配置的 Cloudflare HTTPS endpoint 连回 Mac 后，内容自动同步到 Mac 服务端作为长期归档源。Tailscale 只作为紧急备选和专项排障路径。
...
- 希望内容不依赖第三方社交平台。
- 希望外出时也能通过 Cloudflare HTTPS endpoint 连回 Mac，Tailscale 仅作为紧急备选。
- 后续可能开源给其他个人自托管用户。
...
Mac 默认通过受限的 Cloudflare Tunnel HTTPS endpoint 被 iPhone 访问；该 endpoint 只暴露 iOS 同步所需 API，不开放完整 Admin UI。Tailscale 仅作为紧急备选。
...
- 受限 Cloudflare Tunnel HTTPS endpoint 作为作者真实设备主路径。
- Tailscale 作为紧急备选和专项排障路径。
- 单用户密码登录。
- 长期 device token。
- 设备撤销。
- Mac 文件权限。
...
3. iPhone 通过 Cloudflare HTTPS endpoint 重新连上 Mac 后，App 自动同步。
