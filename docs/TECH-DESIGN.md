...
Calendar v1 完全由 iPhone 本地 Timeline 数据派生，不增加 SQLite schema、sync operation、server calendar API 或 Mac 统计缓存。它默认显示当前月份，支持左右箭头、横向滑动和 `Today` 返回当前月；月份切换使用跟随方向的左右滑入/滑出过渡，而不是淡入淡出；月份网格固定为 42 个日期格，包含空月份和相邻月份日期。Weekday 顺序跟随 `Calendar.current.firstWeekday`，month/day 文案跟随 App Language。
