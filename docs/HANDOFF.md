...
- 2026-05-08 新开置顶功能设计 worktree：`/Users/popcornnnnnn/.codex/worktrees/private-moments-pinned`，分支 `codex/pinned-moments-design`，基于创建时的 `main` commit `6eda03e`。该 checkpoint 只做 M011 设计和文档，不改代码、不运行 live server、不安装真机 App；`main` 可以继续独立推进，后续实现/合并前再按当时的 `main` 状态 rebase 或 merge。
- M011 Pinned Moments 实现 checkpoint：Pinned 是 Timeline 顶部的快捷层，不是第二条 feed。置顶 moment 默认只以折叠 `Pinned · N` 入口出现；当 Pinned 区域可见时，置顶项从无筛选主 Timeline 普通列表中移除，避免重复完整 row。Calendar、Day Review、搜索/筛选、Review 输入和 detail identity 仍按原 `occurredAt` / post id 处理。实现使用 `isPinned` / `pinnedAt` post metadata、`update_post_pin` operation 和 `post_pin_updated` server change。真实设备安装前必须先有 Sync Health/outbox 检查和恢复点。
- 2026-05-08 M011 grill-me 后修正：Pinned 只在无搜索/筛选的主 Timeline 顶部出现，默认只显示 `Pinned · N` header；1-3 条可展开标题行并本机记住，超过 3 条点 header 打开底部 sheet，sheet 内 push 完整 Moment Detail。Pin/Unpin 入口为 Detail `More`、Timeline row context menu、pinned sheet row context menu；Pin 与 Favorite 独立，pending/failed/partial/synced 都允许本地 pin/unpin。非 main worktree 验证默认用模拟器看 UI/交互，sync/data 正确性仍必须用测试和隔离 `3211` server/data smoke 证明，不装真机。
- M011 当前验证：`server:typecheck`、`server:test`、`server:build`、`admin:build`、generic iOS build、iOS simulator tests、隔离 `3211` sync smoke、专用模拟器 UI 验证均已通过；`docs/UAT-GATES.md` 新增 open `UAT-M011-PINNED-MOMENTS`，等待后续真机验收。
...
- M011 如果进入实现，先做 S01/S02 的 schema/sync/local model 小闭环，并用独立端口和 worktree-local data directory 做 server smoke。不要从该 worktree 运行 `npm run ios:device`，除非已经记录真机安装恢复点；可先跑 generic iOS build。
