...
- M011 Pinned Moments 实现 checkpoint：Pinned 是 Timeline 顶部的快捷层，不是第二条 feed。置顶 moment 默认有折叠 `Pinned · N` 快捷入口，同时仍保留在普通 Timeline 的原时间位置，并用轻量 pin 图标标识状态。Calendar、Day Review、搜索/筛选、Review 输入和 detail identity 仍按原 `occurredAt` / post id 处理。实现使用 `isPinned` / `pinnedAt` post metadata、`update_post_pin` operation 和 `post_pin_updated` server change。真实设备安装前必须先有 Sync Health/outbox 检查和恢复点。
