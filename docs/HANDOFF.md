...
- 2026-05-07 Moment Detail 标签展示修复：Tags section 不再复用 Timeline 的单行截断 chip；Detail 使用专用 tag badge，标签名完整显示并可在可用宽度内换行，`Manual` / `AI` 来源移到 chip 下方，避免遮挡或把 `Study` / `RSA...` 等标签截成省略号。generic iOS Debug build 通过，已 build/sign/install/launch 到 `wwz 的 iphone`。
...
- 用一条 fresh clear-speech audio/video 验证新流程：发布后 iOS 不出现 transcript 或 Summary 占位；Mac 后台生成成功后，follow-up sync 让 timeline 出现 `Summary ready`；打开 bottom sheet 显示 v4 document summary，确认短标题、折叠详情、`AI suggested` 标识、Regenerate in-flight 反馈、退出 sheet 后 timeline `Regenerating` 状态、失败保留旧摘要语义、Delete 可用。对 fresh audio 额外确认：如果发布时没有手写 `#`/`##` 标题，AI title 自动插入为 timeline/detail 顶部标题，不显示 `Edited`，Regenerate 不覆盖已有标题；关闭 Settings > Feature Modules > `AI Title Auto-Insert` 后不再对未来音频自动插入。对 AI topic tag 额外确认：新语音应优先复用已有 topic/alias，不为同一概念创建近义重复 tag。
