...
- M011 Pinned Moments 设计结论：Pinned 是 Timeline 顶部的快捷层，不是第二条 feed。置顶 moment 默认以折叠标题行显示，原 moment 仍留在正常时间顺序中；后续实现应使用 `isPinned` / `pinnedAt` post metadata、`update_post_pin` operation 和 `post_pin_updated` server change。真实设备安装前必须先有 Sync Health/outbox 检查和恢复点。
