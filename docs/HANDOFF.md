...
- 2026-05-06 公网 fallback 同步事故定位到 Mac 侧 Cloudflare Tunnel 断连：iPhone 当天有 pending outbox/media，但 Tailscale 设备离线，fallback 域名返回 Cloudflare `530` / `1033`。本机 Clash Verge / Mihomo TUN fake-IP 曾把 `region*.v2.argotunnel.com`、`cloudflare.com`、`cftunnel.com` 和个人 fallback 域名解析到 `198.18.x.x`，导致 `cloudflared` 无法建立 tunnel。修复方式是把 Cloudflare Tunnel 相关域名和个人 fallback 域名加入 Clash `fake-ip-filter` 与直连规则，reload Clash、刷新 DNS cache、重启 `com.popcornnnnnn.cloudflared.blog`；验证时公网 `/api/v1/health` 已恢复 `200`，`/admin/` 仍按 tunnel ingress 返回 `404`。
