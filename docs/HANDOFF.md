...
- 2026-05-07 Admin 迁移开始落地：新增 `docs/ADMIN-MIGRATION.md`，定义 Mac Admin 最小保留面为 Archive/recovery、promote artifact、runtime truth、maintenance jobs、server logs 和 device emergency；Admin 默认入口已从 `Posts` 改为 `Archive`，tab 顺序调整为 `Archive / Overview / Posts`。后续不要继续把日常设置、监控、诊断默认塞进 Admin；优先迁移到 iOS Settings / Diagnostics。
...
- Admin 迁移边界见 `docs/ADMIN-MIGRATION.md`。下一步优先收缩 Overview 为 Runtime / Jobs / Emergency devices；再把 Posts 降为 Debug/Emergency，而不是继续扩展为内容管理台。扩展 Posts management 前，先把 Admin UI 拆成更小的 React components。
