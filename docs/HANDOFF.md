...
- 已通过 generic iOS Debug build：`CODE_SIGNING_ALLOWED=NO`。
- 已通过 timeline 静态 non-clutter 检查：`TimelineRow.swift` 和 `TimelineView.swift` 中没有 `comment` / `Comment` 命中。
- `npm run ios:device` 已成功 build、sign、install 并 launch 到 `wwz 的 iphone`，脚本提示 app Settings 使用 `http://100.114.41.22:3210` 和 `server/.env` 中的密码。
