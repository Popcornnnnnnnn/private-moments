...
- 主时间线和 Day Review 最多显示一个主标签；Moment Detail 只在标签显示开启时展示完整标签和单条编辑入口。Detail 标签 badge 必须完整展示标签名，允许换行，不用省略号，也不让 `Manual` / `AI` 来源文字遮挡标签。`Show Tags in Timeline` 关闭后，标签系统仍然正常参与搜索、筛选、Settings 管理、AI 自动标签和同步，但阅读界面不再露出标签操作。
