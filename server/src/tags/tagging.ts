...
export async function applyAITagsFromSummary(
...
): Promise<{ appliedPrimary: number; appliedTopics: number; skippedReason: string | null }> {
  if (input.forceRegenerate) {
    return { appliedPrimary: 0, appliedTopics: 0, skippedReason: "force_regenerate" };
  }

  if (input.mediaKind !== "audio") {
    return { appliedPrimary: 0, appliedTopics: 0, skippedReason: "non_audio_media" };
  }
  ...
  if (!post || post.deletedAt) {
    return { appliedPrimary: 0, appliedTopics: 0, skippedReason: "post_missing" };
  }

  if (post.tagsUserEditedAt) {
    return { appliedPrimary: 0, appliedTopics: 0, skippedReason: "user_edited" };
  }

  if (post.aiTagProcessedAt) {
    return { appliedPrimary: 0, appliedTopics: 0, skippedReason: "already_processed" };
  }
