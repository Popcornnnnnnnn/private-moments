...
这次公开版同步了最近一轮产品迭代，重点不是把时间线做复杂，而是减少记录和回看时的摩擦：

- **Save to Moments Share Extension**
  可以从 Photos、Files、Voice Memos、Safari 等系统 Share Sheet 直接把图片、视频、音频、文本或链接送进 Moments，主 App 打开后进入原来的 Composer 继续编辑和发布。

- **Smart Tags**
  支持少量预设主标签、动态新增 topic tags、别名、归档、恢复、删除和批量管理。新语音 moment 在 AI summary ready 后可以自动生成主标签和保守的 topic tags；图片、视频和纯文字不会自动打 AI 标签。

- **AI Summary v3 与 AI 标题**
  新语音/视频摘要使用更结构化的 document blocks。对于没有手写标题的新语音，AI 生成的短标题可以自动写入正文顶部，只写标题，不把摘要正文混进原始 moment。

- **更安静的界面开关**
  Settings 里加入标签显示、AI 标题写入等 feature toggles，以及 System / Light / Dark 外观选择。主时间线仍然默认保持简洁。
...
- **系统分享入口**
  从其他 iOS App 的 Share Sheet 选择 `Save to Moments`，把外部内容直接带进 Moments Composer，不需要先打开应用再重新选择素材。
...
- **长语音/视频的 AI 摘要**
  语音或视频上传到 Mac 后，可以先在 Mac 本地转写，再调用外部 OpenAI-compatible API 生成结构化摘要。新语音还可以把 AI 生成的短标题写入正文顶部。iPhone 不保存外部 API key。

- **Smart Tags**
  支持手动主标签和 topic tags；新语音可以在 AI summary ready 后自动生成保守标签。标签可以隐藏在主时间线之外，也可以在 Settings 里管理颜色、别名、归档、合并和删除。

- **搜索与整理**
  时间线支持轻量模糊搜索、媒体类型筛选、月份筛选、收藏筛选、评论筛选、标签筛选、待同步筛选，以及命中来源筛选。
...
- 文字、图片、语音、视频、系统分享导入、评论、收藏、筛选和搜索；
- Smart Tags、标签管理和标签搜索；
- 基于 Mac-local transcription + external summary provider 的 AI media summaries、AI tag suggestions 和 AI title auto-insert；
...
Share Extension 使用 iOS App Group 在扩展和主 App 之间传递临时导入内容。公开版默认 bundle id 是 `dev.privatemoments.app`，默认 App Group 是 `group.dev.privatemoments.app`；真机签名时需要在自己的 Apple Developer 账号里配置对应能力，或替换为你自己的 identifiers。
...
新生成的摘要会尽量给出短标题、一句话总结和分段内容。对于没有手写标题的新语音，App 可以把短标题作为 `##` 标题插入 moment 正文；这个开关可以在 Settings 里关闭。
