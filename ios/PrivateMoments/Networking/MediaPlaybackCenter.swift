...
final class MediaPlaybackCenter: ObservableObject {
    ...
    func toggle(media: TimelineMedia, url: URL) {
        ...
        play(media: media, url: url)
    }

    func toggleActive(mediaId: String) -> Bool {
    ...
    func play(media: TimelineMedia, url: URL, startAt: Double? = nil) {
        if activeMediaId == media.id {
            if let startAt {
                seek(to: startAt)
            }
        ...
        currentTime = startAt ?? savedProgress(for: media.id)
