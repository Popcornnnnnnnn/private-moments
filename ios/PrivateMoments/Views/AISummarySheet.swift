...
struct AISummarySheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .alert("Delete summary?", isPresented: $isDeleteAlertPresented) {
                Button("Cancel", role: .cancel) {}
                Button("Delete", role: .destructive) {
                    onDelete()
                }
            } message: {
                Text("This removes only the generated AI summary.")
            }
    ...
    private var regenerateButtonLabel: some View {
        Label {
            Text(isGenerationActive ? "Regenerating" : "Regenerate")
        } icon: {
            if isGenerationActive {
                ProgressView()
                    .controlSize(.small)
    ...
    private var generationStatusMessage: String {
        if displaySummary != nil {
            return "The current summary will update when the new result is ready."
        }
    ...
    private var failureMessage: String {
        ...
        if displaySummary != nil {
            return "The previous summary is still available. Try regenerating again when the Mac is reachable."
        }

        return "Try regenerating again when the Mac is reachable."
    }
