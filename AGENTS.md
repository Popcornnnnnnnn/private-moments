...
- Smart Tags are first-class synced metadata. Tags use primary/topic roles, Settings > Tags manages vocabulary, aliases, archive/restore/delete, topic merge, primary colors with muted/high-saturation presets plus HEX input, and batch primary color edits. Timeline optionally shows only the primary tag, and new audio moments may receive AI suggested tags from the first ready summary. AI topic tags must prefer existing active topic tags/aliases before new tag creation, including obvious narrower variants such as `HTTPS 中间人攻击` -> `中间人攻击`. Video/image/text moments do not run AI auto-tagging in the current version.
