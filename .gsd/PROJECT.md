...
- [ ] M002: Private comments for moments — in progress. S01 delivered synced private-comment persistence and idempotent create/delete operations across server and iOS storage; S02 delivered the iOS Moment detail UI for viewing, adding, and confirmed deletion while keeping the main timeline uncluttered; S03 closed validation/documentation with server build, iOS simulator tests, generic iOS build, static timeline non-clutter checks, real-device install/launch proof, and durable PRD/TECH-DESIGN/OPERATOR-RUNBOOK/INTEGRATION-GUIDE/sync protocol updates. Manual real-device comment create/delete UAT and active schema-version-4 server SQLite aggregate proof remain follow-up validation for R008.
